...
public class VanillaMetrics {
    ...
    private ConsoleErrorAppender errorAppender;
    private Spark spark;

    private static final TachyonLogger LOGGER = Plugin.getModuleLogger("VanillaMetrics");
    ...
    private void attachLog4jAppender() {
        ...
        errorAppender = new ConsoleErrorAppender("TachyonMetricsAppender");
        errorAppender.start();
    ...
    private void detachLog4jAppender() {
        if (errorAppender != null) {
            Logger rootLogger = (Logger) LogManager.getRootLogger();
            rootLogger.removeAppender(errorAppender);
            errorAppender.stop();
        }
    }

    private class ConsoleErrorAppender extends AbstractAppender {
        ConsoleErrorAppender(String name) {
        ...
        public void append(LogEvent event) {
            ...
            if (level == Level.ERROR || level == Level.FATAL || level == Level.WARN) {
                CONSOLE_LOGS.labels(serverName, level.name()).inc();
            }
        }
    }
}
