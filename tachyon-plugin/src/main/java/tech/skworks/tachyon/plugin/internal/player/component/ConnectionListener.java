...
public class ConnectionListener implements Listener {
    ...
    private static final TachyonLogger LOGGER = Plugin.getModuleLogger("ConnectionListener");

    public ConnectionListener(ProfileManager profileManager, AuditManager audit, ComponentService componentService, GrpcClientManager grpcClientManager) {
        ...
        profile.saveProfile().whenComplete((result, exception) -> {
            ...
            try {
                ...
                if (!freeResp.getSuccess()) {
                    ...
                    LOGGER.info("Player state freed for {} ({}).", player.getName(), uuid);
                }
            } catch (Exception e) {
                LOGGER.log(Level.WARN, e, "freePlayer() call failed for {} — state will expire via TTL (30s).", uuid);
            }
